---
title: "Cap Proof-of-Work Captcha Integration Guide"
...
The session TTL is set by `CAP_SESSION_TTL_SECONDS` (default `120` s). Every time a vote is accepted, both keys have their TTL reset to this value via `EXPIRE`. A user who votes continuously will never see the captcha dialog again. A session that goes idle for more than the TTL expires naturally.
...
Leave `CAP_API_URL` unset (empty string or absent from the environment). The `hasCaptcha` flag in `features.ts` evaluates to `false`, and the Lua vote script skips session validation entirely.
