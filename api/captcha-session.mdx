...
description: "GET /api/captcha/session reports whether Cap captcha is required and whether the current session cookie is valid. Called on every page load."
---
...
<Expandable title="required: true, valid: true — active session">
  A `cap_session` cookie was sent, it exists in DragonFly, the remaining vote quota is ≥ 1, and the stored IP matches the caller's trusted IP. The widget is **not** shown. The browser can vote immediately using the existing cookie; each accepted vote decrements the quota and resets the TTL (`max(30, CAP_SESSION_TTL_SECONDS)`, default 120 s).
</Expandable>
